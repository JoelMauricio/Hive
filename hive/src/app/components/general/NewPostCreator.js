...
export default function NewPost({ PostId, UserId, User, Message, HasImage, ImageSrc }) {
    ...
    const makePost = async () => {
        let containsImage = selectedImage ? true : false
        const { data, error } = await supabase.from("tblpost").insert({ "author": profile, "content": keyword, "photo": containsImage ? SaveImage() : null, "hasphoto": containsImage })
        if (containsImage) {
            SaveImage();
        }
        setKeyword("");
